...
@Service
public class CommentServiceImpl extends BaseInfoPropertise implements CommentService {
    ...
    public PagedGridResult queryVlogComments(String vlogId,
        ...
        for(CommentVO cv : list){
            ...
            // 当前短视频某个评论点赞总数
            String countsStr = redis.getHashValue(REDIS_VLOG_BE_LIKED_COUNTS, commentId);
            Integer counts = 0;
            ...
            // 判断当前用户是否点赞过该评论
            String doILoke = redis.hget(REDIS_USER_LIKE_COMMENT, userId + ":" + commentId);
            if (StringUtils.isNotBlank(doILoke) && doILoke.equalsIgnoreCase("1")) {
